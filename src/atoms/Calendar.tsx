...
const Calendar:React.FC<CalendarProps> = ({date}) => {       
    ...
    const firstDay = dayjs(currentDate).startOf('month').subtract(parseInt(dayjs(date).startOf('month').format('d')), 'day');
    const cells = dayjs(currentDate).endOf('month').diff(firstDay, 'day') + 1 > 35 ? 42 : 35;
    return (
        ...
        <CalendarContainer cells={cells}>
            <div className="calendar-banner">
                <div className="calendar-previous">
                    ...
                    }}>{dayjs(currentDate).subtract(1, 'month').format('MMM')}</button>
                </div>
                ...
                <div className="calendar-next">
                    ...
                    }}>{dayjs(currentDate).add(1, 'month').format('MMM')}</button>
                </div>
            </div>
            { [...Array(7)].map((_, i) => 
                <div className="calendar-title">{dayjs().startOf('week').add(i, 'day').format('ddd')}</div>
            ) }
            { [...Array(cells)].map((_, i) => 
                <CalendarCell date={firstDay.add(i, 'day')} activeDate={dayjs(currentDate).startOf('day')}/>
            )}
        </CalendarContainer>
    )
}
...
const CalendarCell:React.FC<CalendarCellProps> = ({date, activeDate}) => {
    ...
    return <StyledCalendarCell status={status}><h2>{date.format('D')}</h2></StyledCalendarCell>
}
